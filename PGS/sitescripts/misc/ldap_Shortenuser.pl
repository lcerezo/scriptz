...
open (LOGFILE, ">/tmp/log");
	$ldap = Net::LDAP->new( 'onshore.pgs.com' ) or die "$@";
	$mesg = $ldap->bind ( ' CN=hou-likewise_dirsvc,OU=Service accounts,OU=Houston,DC=onshore,DC=pgs,DC=com' ,	#dn of login user for script, YOU?

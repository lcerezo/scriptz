...
$dmzuser = "totalFR";
$helpurl = "http://pinky.onshore.pgs.com/wiki/index.php/Total-Pau"
$clientsite = "totalPau";
$systype = `uname -p`;
...
#	elsif ($systype =~ /i*86/)	{
#					$holobin = "/tps/holoSeis/Linux/holoSeis.9.40.19";
#					$holoroot = "/tps/holoSeis/Linux";
#					$mpconfig = "/tps/holoSeis/Linux/.mpconfig.remote.1920x1200";
#					}
#
print "This will setup the ssh tunnels and run the proper holoSeis command for sharing.\n Full docs avail at $helpurl.\n\n";
#
print "Please enter \"2\" if you will only need a two-way sharing with $clientsite, Enter \"3\" if a three-way\t>> ";
$tuntype = <STDIN>;
chomp ($tuntype);
...
sub holoshare2	{
		system  "ssh -f -N -R 9991:localhost:$serverport -l $dmzuser $dmzbox";
		system  "ssh -f -N -L 9003:localhost:9993 -l $dmzuser $dmzbox";
		system "$holobin -root $holoroot  -mpcfile $mpconfig -server\:$serverport -client\:localhost\:9003 -nosettings $dataset";
		}	
sub holoshare3	{
...
sub getpgspartner
		...
		system "ssh -f -N -R $remoteport:localhost:$serverport -L 9003\:localhost\:9993 -l $dmzuser $dmzbox";
		}

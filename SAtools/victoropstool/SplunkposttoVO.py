...
class PostSplunkAlertToVictorOps(object):
    """ this is to take the stuff from Splunk triggered alerts and victor ops them"""
    ...
    def post_message(self, myurl, payload):
        ...
        try:
            ...
            post_error = '''Error posting to VictorOps %s \n\nresponse was %s\n
            HTTP code was %s\n\n''' % (
                err,
                response.text,
...
if __name__ == "__main__":
    try:
        MY_INCIDENT = PostSplunkAlertToVictorOps()
        THIS_URL = MY_INCIDENT.gen_url(VOAPIKEY, ROUTING_KEY)
        MY_PAYLOAD = MY_INCIDENT.build_json_object(
        ...
        HTTP_RESPONSE_TEXT, HTTP_RESPONSE_CODE = MY_INCIDENT.post_message(THIS_URL, MY_PAYLOAD)
